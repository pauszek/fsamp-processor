...
def get_file_processor() -> FileProcessorService:
    ...
    logger.info("FileProcessorService initialized successfully",
                outbox_enabled=outbox_repo is not None)
    return _file_processor
...
def record_handler(record: SQSRecord) -> dict[str, Any]:
    ...
    try:
        ...
        metrics.add_dimension(name="MimeType", value=mime_type.split("/")[0] if "/" in mime_type else mime_type)
        ...
        total_duration_ms = int((time.time() - start_time) * 1000)
        metrics.add_metric(name="FailedProcessingDuration", unit=MetricUnit.Milliseconds, value=total_duration_ms)
        ...
        logger.error("Processing error (retryable)", 
                    error=str(e), 
