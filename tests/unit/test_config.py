...
def test_should_use_fips_requires_us_region_and_no_custom_endpoint() -> None:
    assert Settings(environment="prod", aws_region="us-east-1").should_use_fips is True
    assert Settings(environment="prod", aws_region="eu-west-1").should_use_fips is False
    assert (
        Settings(
            environment="prod",
